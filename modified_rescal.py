...
#
# FORKED EXACTLY AS IS FROM github.com/mnick/rescal.py with a regularization term
# added to perform Semantically Smooth Embedding (SSE) as shown by Gao et al.
...
def als(X, rank, **kwargs):
    ...
    # Create the W matrix for SSE regularization
    W = np.zeros((n,n))

    # w_ij = 1 iff \sum_k X_ijk \geq 1
    for curK in range(k):
        curMat = X[curK]
        nonzeros = curMat.nonzero()
        rows = nonzeros[0]
        cols = nonzeros[1]
        # there are as many edges as len(rows) = len(cols)
    ...
    for itr in range(maxIter):
        ...
        # compute fit value
        if compute_fit:
            fit = _compute_fit(X, A, R, P, Z, W, lmbdaA, lmbdaR, lmbdaV)
        else:
            fit = itr
...
def _compute_fit(X, A, R, P, Z, W, lmbdaA, lmbdaR, lmbdaZ):
    ...
    # Add the regularization term on the fit computation
    # g = \sum_n || A_i - \sum_j w_ij A_j || = \sum_n || A_i - W_i \dot A ||
    for i in range(n):
        sumVec = dot(W[i], A)
        g += norm(A[i] - sumVec) ** 2

    return 1 - (f + g) / sumNorm
